...
Employee* createEmployees(char names[][50], double salaries[], int empCount) { 
  ...
  for(i = 0; i < empCount; ++i){
    ...
    pEmployees[i].name = (StringType*)malloc(sizeof(StringType));
    pEmployees[i].name->length = strlen(names[i]);
    pEmployees[i].name->string = (char*)malloc(sizeof(char) * (pEmployees[i].name->length + 1));
    strcpy(pEmployees[i].name->string, names[i]);
    pEmployees[i].salary = salaries[i];    
  }
  return pEmployees;
