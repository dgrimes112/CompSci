...
def result():
    ...
    if len(l1) == len(l2) and functools.reduce(lambda x, y : x and y, map(lambda p, q: p == q, l1, l2), True): 
        ...
        subprocess.run(["rm program4solution_student_output.txt"], capture_output=True, text = True, shell = True)
    else: 
        print("  ------  ")
        print(" / o  o \ ")
        print("|        |")
...
def compilerunfile():
    ...
    javac = "javac " + cwd + "/HopStepGameDriver.java" #compile solution file command and generate class file

    proc = subprocess.run([javac], capture_output=True, text = True, shell = True)
    

    
    java = "java HopStepGameDriver"
    ...
    try:
        proc2 = subprocess.run([java], capture_output=True, input = inputs, text = True, shell = True, timeout = 30) 
...
def main():
    ...
    subprocess.run(["rm HopStepGameDriver.class"], capture_output=True, text = True, shell = True)
    subprocess.run(["rm HopStepGame.class"], capture_output=True, text = True, shell = True)
