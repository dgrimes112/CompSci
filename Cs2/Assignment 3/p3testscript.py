...
def result():
    ...
    if len(l1) == len(l2) and functools.reduce(lambda x, y : x and y, map(lambda p, q: p == q, l1, l2), True): 
        ...
        subprocess.run(["rm program3solution_student_output.txt"], capture_output=True, text = True, shell = True)
    else: 
        print("  ------  ")
        print(" / o  o \ ")
        print("|        |")
...
def compilerunfile():
    ...
    try:
        ...
        raise TimeoutExpired("Your program exceeded the time... Submissions like this will receive no credit in certain categories based on the assignment directions.")
...
def main():
    ...
    subprocess.run(["rm TomatoesDriver.class"], capture_output=True, text = True, shell = True)
    subprocess.run(["rm Tomatoes.class"], capture_output=True, text = True, shell = True)
