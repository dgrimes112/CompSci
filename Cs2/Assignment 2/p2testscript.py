...
def result():
    ...
    if len(l1) == len(l2) and functools.reduce(lambda x, y : x and y, map(lambda p, q: p == q, l1, l2), True): 
        ...
        subprocess.run(["rm program2solution_student_output.txt"], capture_output=True, text = True, shell = True)
    else: 
        print("  ------  ")
        print(" / o  o \ ")
        print("|        |")
...
def main():
    ...
    subprocess.run(["rm TreasureCoordinatesDriver.class"], capture_output=True, text = True, shell = True)
    subprocess.run(["rm TreasureCoordinates.class"], capture_output=True, text = True, shell = True)
